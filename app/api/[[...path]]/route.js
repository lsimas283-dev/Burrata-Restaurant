...
function allowedOrigins() {
  return (process.env.CORS_ORIGINS || process.env.APP_ORIGIN || '').split(',').map((v) => v.trim()).filter(Boolean);
}
...
function tooMany(retryAfter) {
  const response = NextResponse.json({ error: 'too many requests' }, { status: 429, headers: { ...jsonHeaders, 'Retry-After': String(retryAfter) } });
  return response;
}
...
async function adminAllowed(request) {
  return !rejectOrigin(request) && isAdminAuthenticated(request);
}
...
export async function GET(request, { params }) {
  try {
    ...
    if (path === '' || path === 'health') {
      return NextResponse.json({ ok: true, service: 'Burrata Empório & Bistrô API' }, { headers: jsonHeaders });
    }
    if (path === 'admin/session') {
      return NextResponse.json({ authenticated: isAdminAuthenticated(request) }, { headers: jsonHeaders });
    }
    if (path === 'menu') {
    ...
    if (path === 'reservations') {
      if (!(await adminAllowed(request))) return unauthorized();
...
export async function POST(request, { params }) {
  try {
    ...
    if (path === 'admin/login') {
      if (rejectOrigin(request)) return unauthorized();
      const limit = await checkRateLimit(db, `admin-login:${getClientIp(request)}`, 5, 15 * 60 * 1000);
      if (!limit.allowed) return tooMany(limit.retryAfter);
      const body = await parseJson(request);
      if (!body || typeof body.password !== 'string' || body.password.length < 1 || body.password.length > 256) return badRequest();
      const valid = body.password === process.env.ADMIN_PASSWORD;
      if (!valid) return unauthorized();
      const response = NextResponse.json({ ok: true }, { headers: jsonHeaders });
      return setAdminSessionCookie(response, createAdminSession());
    }
    ...
    if (path === 'reservations') {
      if (rejectOrigin(request)) return unauthorized();
      const limit = await checkRateLimit(db, `reservation:${getClientIp(request)}`, 10, 10 * 60 * 1000);
      if (!limit.allowed) return tooMany(limit.retryAfter);
      const body = await parseJson(request);
      const parsed = reservationSchema.safeParse(body);
      if (!parsed.success) return badRequest();
      const doc = {
        id: uuidv4(), ...parsed.data, status: 'pending', seen: false, createdAt: new Date().toISOString(),
      };
    ...
    if (path === 'menu') {
      const parsed = menuSchema.safeParse({ order: body.order ?? 9999, ...body });
      if (!parsed.success) return badRequest();
      const doc = { id: uuidv4(), ...parsed.data, createdAt: new Date().toISOString() };
      await db.collection('menu_items').insertOne(doc);
      return NextResponse.json({ ok: true, item: doc }, { headers: jsonHeaders });
    }

    if (path === 'gallery') {
      const parsed = gallerySchema.safeParse({ order: body.order ?? 9999, ...body });
      if (!parsed.success) return badRequest();
      const doc = { id: uuidv4(), ...parsed.data, createdAt: new Date().toISOString() };
...
export async function PUT(request, { params }) {
  try {
    if (rejectOrigin(request) || !isAdminAuthenticated(request)) return unauthorized();
    const path = ((await params).path || []).join('/');
    const db = await getDb();
    const body = await parseJson(request);
    if (!body) return badRequest();
    ...
    if (parts[0] === 'menu' && parts[1]) {
      ...
      if (!parsed.success) return badRequest();
      await db.collection('menu_items').updateOne({ id: parts[1] }, { $set: parsed.data });
      const item = await db.collection('menu_items').findOne({ id: parts[1] }, { projection: { _id: 0 } });
      return NextResponse.json({ ok: true, item }, { headers: jsonHeaders });
    }

    if (parts[0] === 'gallery' && parts[1]) {
      const parsed = galleryUpdateSchema.safeParse(body);
      if (!parsed.success) return badRequest();
      await db.collection('gallery').updateOne({ id: parts[1] }, { $set: parsed.data });
      return NextResponse.json({ ok: true }, { headers: jsonHeaders });
    }
    ...
    if (parts[0] === 'reservations' && parts[1]) {
      ...
      await db.collection('reservations').updateOne({ id: parts[1] }, { $set: update });
